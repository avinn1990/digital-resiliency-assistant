...
Deployed as static site `dra-ui` via root `render.yaml`. `VITE_API_URL` must be the **public** `dra-backend` URL (`https://….onrender.com`). After changing it on Render, **manual redeploy dra-ui** (Vite bakes it at build time).
